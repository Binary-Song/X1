...
{
public:
    static void Log(_X1LogExtras type, const char *file, int line,
...
#define _X1_ASSERT(type, ret, ...)                                              \
    _X1_ASSERT_GET_OVLD(__VA_ARGS__, _X1_ASSERT_FMT2, _X1_ASSERT_FMT2,         \
                        _X1_ASSERT_FMT2, _X1_ASSERT_FMT1, _X1_ASSERT_FMT0)     \
    (type, ret, __VA_ARGS__)

// clang-format off
#define _X1_LOGTYPE_AS _X1LogExtras(_X1LogType::Assert)
#define _X1_LOGTYPE_LG _X1LogExtras(_X1LogType::Log)
#define _X1_LOGTYPE_LR _X1LogExtras(_X1LogType::Log, true)
#define _X1_FAILACT_RV return
#define _X1_FAILACT_RE return {}
#define _X1_FAILACT_CO continue
#define _X1_FAILACT_PA ;
...
#define X1_ASSERT_RET_VOID(...)     _X1_ASSERT(_X1_LOGTYPE_AS, _X1_FAILACT_RV, __VA_ARGS__)
// 当条件不满足时，在屏幕和Log输出消息，并从当前函数返回 {}（不会导致程序崩溃）。
// 示例语法：
// (1) (Foo>0)
// (2) (Foo>0, "Foo must be greater than 0!")
// (3) (Foo>0, "Foo={0}. Foo must be greater than 0!", Foo)
#define X1_ASSERT_RET_EMPTY(...)    _X1_ASSERT(_X1_LOGTYPE_AS, _X1_FAILACT_RE, __VA_ARGS__)
// 当条件不满足时，在屏幕和Log输出消息，并 continue （不会导致程序崩溃）。
// 示例语法：
// (1) (Foo>0)
// (2) (Foo>0, "Foo must be greater than 0!")
// (3) (Foo>0, "Foo={0}. Foo must be greater than 0!", Foo)
#define X1_ASSERT_CONTINUE(...)     _X1_ASSERT(_X1_LOGTYPE_AS, _X1_FAILACT_CO, __VA_ARGS__)
// 当条件不满足时，在屏幕和Log输出消息，并继续执行（不会导致程序崩溃）。
// 示例语法：
// (1) (Foo>0)
// (2) (Foo>0, "Foo must be greater than 0!")
// (3) (Foo>0, "Foo={0}. Foo must be greater than 0!", Foo)
#define X1_ASSERT_PASS(...)         _X1_ASSERT(_X1_LOGTYPE_AS, _X1_FAILACT_PA, __VA_ARGS__)
// 在屏幕和Log输出消息。重复消息不会显示。
// 示例语法：
// (1) ("Hello world!")
// (2) ("Hello world! Foo={0}!", Foo)
#define X1_LOG(...)            _X1_ASSERT(_X1_LOGTYPE_LG, _X1_FAILACT_PA, 0, __VA_ARGS__)
// 在屏幕和Log输出消息。重复消息会显示。
// 示例语法：
// (1) ("Hello world!")
// (2) ("Hello world! Foo={0}!", Foo)
#define X1_LOG_REPEAT(...)     _X1_ASSERT(_X1_LOGTYPE_LR, _X1_FAILACT_PA, 0, __VA_ARGS__)
// clang-format on
